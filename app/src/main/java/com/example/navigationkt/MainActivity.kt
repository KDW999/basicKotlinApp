...
class MainActivity : AppCompatActivity(), NavigationView.OnNavigationItemSelectedListener {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        naviView.setNavigationItemSelectedListener(this) // 네비게이션 메뉴 아이템에 클릭 속성 부여, 이게 있어야 밑에 적어 놓은 기능들이 실행된다
    }
    ...
    override fun onBackPressed() { // 백버튼 누를 시 수행하는 메소드
        if(layout_drawer.isDrawerOpen(GravityCompat.START)) // 메뉴가 켜져있다면
        {
            layout_drawer.closeDrawers()
        }
        else { // 메뉴가 안켜져있을 때 백버튼 누르면 앱이 꺼지게 된다.
