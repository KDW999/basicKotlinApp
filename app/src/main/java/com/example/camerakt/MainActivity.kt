...
class MainActivity : AppCompatActivity() {
    ...
    lateinit var curPhotoPath: String // 문자열 형태의 사진 경로 값 (초기 값을 null로 시작하고 싶을 때)
                                      // java의 int a 처럼 a값을 지정하지 않고 a를 선언만 하고 싶을 때 사용
    ...
    private fun createImageFile(): File? {
        val timestamp: String = SimpleDateFormat("yyyyMMdd_HHmmss").format(Date()) //날짜 형식 지정, 사진을 저장할 때 날짜를 지정하기 위해 사용하였음
        val storageDir: File? = getExternalFilesDir(Environment.DIRECTORY_PICTURES) // ?를 사용하는 이유 -> 파일을 가져올 때 실패하는 경우를 대비해 파일이 null일수도 있다는 걸 알려줌
        return File.createTempFile("JPEG_${timestamp}_", ".jpg", storageDir) // -> 자바의 "a"+"b"와 같음 코틀린은 ${}로 문자열 합친다
    ...
    private fun setPermission() {
        val permission = object : PermissionListener {
            ...
            override fun onPermissionDenied(deniedPermissions: MutableList<String>?) { //설정해놓은 위험 권한들이 거부 되었을 경우 이 곳을 수행
                Toast.makeText(this@MainActivity, "권한이 허용 되었습니다.", Toast.LENGTH_SHORT).show()
            }
        }
